...
def _make_card(db, subject="spanish", topic="basics", term="hola", definition="hello") -> Card:
    deck = db.query(Deck).filter(
        Deck.category == "language", Deck.subject == subject, Deck.topic == topic
    ).first()
    if not deck:
    ...
    card = Card(
        ...
        idempotency_key=compute_idempotency_key("language", subject, topic, term),
    )
    db.add(card)
    db.commit()
    return card
...
def _add_log(db, days_ago: int, sessions: int = 1, cards_seen: int = 5) -> None:
    db.add(StudyLog(user_id=_USER_ID, study_date=date.today() - timedelta(days=days_ago), sessions=sessions, cards_seen=cards_seen))
    db.commit()
...
def test_upsert_increments_times_seen(db_session):
    ...
    upsert_card_stat(db_session, _USER_ID, card.id, "word_to_meaning", is_correct=False)
    upsert_card_stat(db_session, _USER_ID, card.id, "word_to_meaning", is_correct=False)
...
def test_upsert_increments_times_correct_only_when_correct(db_session):
    card = _make_card(db_session)
    upsert_card_stat(db_session, _USER_ID, card.id, "word_to_meaning", is_correct=True)
    upsert_card_stat(db_session, _USER_ID, card.id, "word_to_meaning", is_correct=False)
    upsert_card_stat(db_session, _USER_ID, card.id, "word_to_meaning", is_correct=True)
    stat = db_session.query(CardStat).filter_by(card_id=card.id, direction="word_to_meaning").first()
...
def test_upsert_two_directions_produce_two_rows(db_session):
    card = _make_card(db_session)
    upsert_card_stat(db_session, _USER_ID, card.id, "word_to_meaning", is_correct=True)
    upsert_card_stat(db_session, _USER_ID, card.id, "meaning_to_word", is_correct=False)
...
def test_get_weakest_cards_aggregates_across_directions(db_session):
    ...
    result = get_weakest_cards(db_session, _USER_ID)
    assert len(result) == 1
...
def test_get_weakest_cards_high_dir_weakness_ranked_correctly(db_session):
    good = _make_card(db_session, term="bueno", definition="good")
    struggling = _make_card(db_session, term="malo", definition="bad")

    for _ in range(5):
        upsert_card_stat(db_session, _USER_ID, good.id, "word_to_meaning", is_correct=True)
    for _ in range(4):
        upsert_card_stat(db_session, _USER_ID, good.id, "meaning_to_word", is_correct=True)
    upsert_card_stat(db_session, _USER_ID, good.id, "meaning_to_word", is_correct=False)

    for _ in range(5):
        upsert_card_stat(db_session, _USER_ID, struggling.id, "word_to_meaning", is_correct=True)
    for _ in range(5):
        upsert_card_stat(db_session, _USER_ID, struggling.id, "meaning_to_word", is_correct=False)
...
def test_get_weakest_cards_excludes_seen_fewer_than_twice(db_session):
    ...
    once = _make_card(db_session, term="hola", definition="hello")
    upsert_card_stat(db_session, _USER_ID, once.id, "word_to_meaning", is_correct=True)
...
def test_get_weakest_cards_ranks_worst_first(db_session):
    strong = _make_card(db_session, term="hola", definition="hello")
    weak   = _make_card(db_session, term="gracias", definition="thank you")

    for _ in range(4):
        upsert_card_stat(db_session, _USER_ID, strong.id, "word_to_meaning", is_correct=True)
    for _ in range(2):
        upsert_card_stat(db_session, _USER_ID, weak.id, "word_to_meaning", is_correct=False)
...
def test_log_study_day_creates_one_row(db_session):
    log_study_day(db_session, _USER_ID, cards_seen=10)
...
def test_log_study_day_increments_sessions_on_repeat(db_session):
    log_study_day(db_session, _USER_ID, cards_seen=10)
    log_study_day(db_session, _USER_ID, cards_seen=5)
...
def test_get_streak_returns_zero_with_no_logs(db_session):
    result = get_streak(db_session, _USER_ID)
